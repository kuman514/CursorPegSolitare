...
- `main` 엘리먼트 스타일링에 `.main`이라는 클래스가 아닌 그냥 `main`이라는 태그를 선택
- import할 레이어 순서가 역전되었거나 동일 레이어 내 다른 슬라이스를 import한 부분 조정하기
- 게임 오버 오버레이 텍스트
  - 크기 `4rem`
  - 굵기 `700`
- `if-else`문보단 `switch-case`문을 사용
  - `src/widgets/game-over-overlay.tsx`의 37~44번째 줄
  - `src/widgets/tile-button.tsx`의 `boardState`의 값에 대한 모든 분기
...
- 사용하지 않은 import 또는 변수/함수 선언 지우기
